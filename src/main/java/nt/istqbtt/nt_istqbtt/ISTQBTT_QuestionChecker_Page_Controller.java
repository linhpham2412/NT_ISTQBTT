...
public class ISTQBTT_QuestionChecker_Page_Controller implements Initializable {
    public Button btn_QuestionCheckerStartTest;
    public ComboBox selectQuesitonCheckerTestingTypeComboBox;
    public TextField testNameTextField;
    public Pane questionCheckerPagePane;
    public VBox questionCheckerPageVBox;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        try {
            ...
            File questionZipFile = fileChooser.showOpenDialog(null);
            questionFileName = questionZipFile.getName().substring(0, questionZipFile.getName().indexOf("."));
            questionCheckerPageVBox = setupHomePage(toolFont);
        } catch (IOException | ZipException e) {
            throw new RuntimeException(e);
        }
        questionCheckerPagePane.getChildren().add(questionCheckerPageVBox);
    }
...
//            }
        ...
        Label welcomeTitle = new Label("ISTQB Question Checker Tool");
        ...
        selectQuesitonCheckerTestingTypeComboBox = new ComboBox();
        questionHandler.getListOfISTQBTypeReadFromData().stream()
                .map(e -> selectQuesitonCheckerTestingTypeComboBox.getItems().add(e)).collect(Collectors.toList());
        selectQuesitonCheckerTestingTypeComboBox.setStyle("-fx-font-size: 16");
        selectQuesitonCheckerTestingTypeComboBox.setOnAction(event -> {
            questionHandler.questionGroupName = (String) selectQuesitonCheckerTestingTypeComboBox.getValue();
            istqbDetailText.setText(selectQuesitonCheckerTestingTypeComboBox.getValue().toString() + "\nNumber of Question: 40\nPassing Score (at least 65%): 26\nTesting Time: 60 minutes\n" +
        ...
        selectTestingTypeHBox.getChildren().add(selectQuesitonCheckerTestingTypeComboBox);
        HBox.setMargin(selectYourTestingTypeLabel, new Insets(10, 10, 10, 10));
        HBox.setMargin(selectQuesitonCheckerTestingTypeComboBox, new Insets(10, 10, 10, 10));
        ...
        btn_QuestionCheckerStartTest = new Button("Start Test");
        btn_QuestionCheckerStartTest.setFont(toolFont);
        btn_QuestionCheckerStartTest.setOnAction(event -> {
            if (selectQuesitonCheckerTestingTypeComboBox.getValue() != null) {
                questionHandler.isFirstLoad = true;
                testUserName = testNameTextField.getText();
                try {
                    changeStageAndScene(event, setupLayoutPageExam(toolFont), "Examination Page of: " + selectQuesitonCheckerTestingTypeComboBox.getValue());
        ...
        commandContainer.getChildren().add(btn_QuestionCheckerStartTest);
        commandContainer.getChildren().add(quitAppHomeButton);
        HBox.setMargin(btn_QuestionCheckerStartTest, new Insets(10, 10, 10, 10));
...
//        VBox.setMargin(qrcode, new Insets(screenHeight / 8, 0, 0, 0));
    ...
    private ScrollPane getQuestionPages(int pageIndex) {
        ...
        questionImage = new ImageView[10];
    ...
    private void addNavigationButtonsToVBoxContainer(VBox questionContainer, int pageIndex) {
        ...
        navigationContainer.setPrefWidth(objectWidthInScrollPane);
        navigationContainer.setMaxWidth(objectWidthInScrollPane);
